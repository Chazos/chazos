...
@section('content')
<main class="h-full overflow-y-auto " x-data="{ acceptsData: 'false', showAddCollectionModel : false,showAddField : false, showCollectionNameForm: true, editCollectionField : false }">
            ...
            <ul class="ml-5">
                @foreach ( $collections as $collection )
                    <li  class="py-3 hover:bg-purple-400 flex flex-row">
                      <button x-on:click="getCollectionDetails({{ $collection->id }})" class="">{{ ucfirst($collection->display_name)  }}</button>
